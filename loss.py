...
def adaptive_sigma(source, target):
    """
...
def maximum_mean_discrepancy( source, target, kernel_mul=2.0, kernel_num=5, bandwidths=[0.1, 0.5, 1.0]):
    """最大均值差异损失"""
    mmd_loss = 0.0

    # 如果没有指定sigma值，使用自适应sigma
    if bandwidths is None:
        sigma = adaptive_sigma(source, target)
        bandwidths = [sigma]

    for sigma in bandwidths:
        # 计算源样本的高斯核
        kernel_ss = gaussian_kernel(source, source, kernel_mul=kernel_mul, kernel_num=kernel_num,
                                         fix_sigma=sigma)
        # 计算目标样本的高斯核
        kernel_tt = gaussian_kernel(target, target, kernel_mul=kernel_mul, kernel_num=kernel_num,
                                         fix_sigma=sigma)
        # 计算源和目标样本之间的高斯核
        kernel_st = gaussian_kernel(source, target, fix_sigma=sigma, kernel_mul=kernel_mul,
                                         kernel_num=kernel_num)
        # 计算目标域和源域之间的高斯核
        kernel_ts = gaussian_kernel(target, source, fix_sigma=sigma, kernel_mul=kernel_mul,
                                         kernel_num=kernel_num)
...
def cosine_similarity_loss( source, target):
    """计算余弦相似度损失"""
...
def wasserstein_distance_loss( source, target):
    """计算Wasserstein距离损失"""
...
def pseudo_label( source, target, source_labels):
    """
    生成目标域伪标签
    :param source: 源域特征
    :param target: 目标域特征
...
def NT_Xent_Loss( source, target, source_labels):
    """
    基于标签的对比学习损失
    """
    ...
    # 生成伪标签
    pseudo_labels = pseudo_label(source, target, source_labels)
    ...
    # 计算损失
    exp_sim = torch.exp(cosine_similarity / 0.5)
...
def gaussian_kernel1( source, target, kernel_mul=2.0, kernel_num=5, fix_sigma=None):
    """
...
def lmmd( source_features, target_features, source_labels, target_preds, num_classes):
    """
    计算LMMD
    ...
    kernels = gaussian_kernel1(source_features, target_features)
...
def gaussian_kernel_matrix(x, y=None, kernel_mul=2.0, kernel_num=5, fix_sigma=None):
    """
    计算 RBF 核矩阵；若 y is None，则对 x 和 x 做配对（即构造 n+n 的矩阵由上游调用）
    参考常见 MMD 实现
    """
    if y is None:
        y = x
    total = torch.cat([x, y], dim=0)  # (n+m, feat_dim)
    ...
    if fix_sigma:
        ...
        # median heuristic
        n = total.size(0)
        # use pairwise distances, avoid 0 on diagonal by adding eps
...
# -------------------------
# JMMD 实现（多层联合分布 MMD）
# layers_src: list of tensors [x_s_layer1, x_s_layer2, ...], each shape (batch_size, feat_dim_l)
# layers_tgt: list of tensors [x_t_layer1, ...], corresponding layers
# 返回标量损失
# -------------------------
def jmmd_loss(layers_src, layers_tgt, kernel_mul=2.0, kernel_num=5, fix_sigma=None):
    """
    计算 Joint MMD：
    对于每个层 l，构建 (Xs_l || Xt_l) 的核矩阵 K_l (2B,2B)
    然后取 joint_kernel = Π_l K_l（逐元素相乘）
    最终 MMD = mean(K_ss) + mean(K_tt) - 2*mean(K_st)
    """
    # 检查
    assert len(layers_src) == len(layers_tgt), "层数不匹配"
    n_layers = len(layers_src)
    batch = layers_src[0].size(0)
    device = layers_src[0].device
    ...
    for l in range(n_layers):
        ...
        xt = layers_tgt[l]  # (B, d)
        # 将特征做 L2 归一化
        xs_norm = F.normalize(xs, dim=1)
        xt_norm = F.normalize(xt, dim=1)
        # 计算当前层的核矩阵（2B x 2B）
        K_l = gaussian_kernel_matrix(xs_norm, xt_norm, kernel_mul=kernel_mul, kernel_num=kernel_num, fix_sigma=fix_sigma)
        if K_all is None:
            K_all = K_l
        else:
            K_all = K_all * K_l  # 逐元素相乘得到 joint kernel
    ...
    K_tt = K[B:, B:]
    K_st = K[:B, B:]
    # unbiased / biased? 使用 biased estimator 的平均值形式
