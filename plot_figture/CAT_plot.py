...
def plot_CAT(source_teacher_acc_list, source_teacher_losses_list, target_teacher_acc_list, target_student_losses_list, test_acc_list, losses_list, cm, save_dir, fault_classes):
    print('plot_acc_loss_CAT')
    plt.figure(1, figsize=(10, 4), dpi=600)
    plt.rcParams['font.sans-serif'] = ['times new roman']
    plt.rcParams['axes.unicode_minus'] = False  # 用来正常显示负号
    plt.subplot(1, 2, 1)
    # 绘制曲线
    plt.plot(source_teacher_acc_list, label='source_train_acc', linestyle='-', color='blue')
    plt.plot(target_teacher_acc_list, label='target_train_acc', linestyle='-', color='green')
    ...
    plt.subplot(1, 2, 2)
    plt.plot(source_teacher_losses_list, label='source_train_loss', linestyle='-', color='blue')
    plt.plot(target_student_losses_list, label='target_train_loss', linestyle='-', color='green')
...
def plot_tsne_CAT(args, save_dir, student_model, teacher_model, student_model_name, teacher_model_name):
    try:
        ...
        student_model.load_state_dict(torch.load(os.path.join(save_dir, student_model_name)))
        teacher_model.load_state_dict(torch.load(os.path.join(save_dir, teacher_model_name)))
        ...
        # Extract features
        val_raw, val_fc, val_label = test_plot(val_loader, student_model,  device)
        src_raw, src_fc, src_label = test_plot(source_loader, teacher_model,  device)
