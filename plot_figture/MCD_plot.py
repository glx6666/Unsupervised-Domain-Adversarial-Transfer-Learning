...
def plot_MCD(source_acc_list, source_losses_list, losses_dis_list, losses_g_list, val_acc_list, val_losses_list, cm, save_dir, fault_classes):
    print('plot_acc_loss_MCD')
    ...
    plt.plot(losses_dis_list, label='D_loss', linestyle='-', color='orange')
    plt.plot(losses_g_list, label='G_loss', linestyle='-', color='red')
...
def plot_tsne_MCD(args, save_dir, feature_extractor_MCD, classification_extractor1, classification_extractor2,
                              feature_extractor_MCD_name, classification_extractor1_name, classification_extractor2_name):
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    print(f"Using device: {device}")
    print('plot_tsne_MCD')
    init_seed(args.seed)
    feature_extractor_MCD.to(device)
    classification_extractor1.to(device)
    classification_extractor2.to(device)
    # Load pretrained weights
    feature_extractor_MCD.load_state_dict(torch.load(os.path.join(save_dir, feature_extractor_MCD_name)))
    classification_extractor1.load_state_dict(torch.load(os.path.join(save_dir, classification_extractor1_name)))
    classification_extractor2.load_state_dict(torch.load(os.path.join(save_dir, classification_extractor2_name)))
    ...
    # Extract features
    val_raw, val_fc, val_label = test_plot(val_loader, feature_extractor_MCD, classification_extractor2,  device)
    src_raw, src_fc, src_label = test_plot(source_loader, feature_extractor_MCD, classification_extractor1,  device)
