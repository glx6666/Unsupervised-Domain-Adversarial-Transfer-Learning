...
"""
source_acc_list[epoch], source_losses_list[epoch],
                                         val_acc_list[epoch], val_losses_list[epoch]
"""

def plot_my(source_train_acc_list, source_total_losses_list, class_losses_list, weighted_domain_losses_list, test_acc_list, losses_list, cm, save_dir, fault_classes):
    print('plot_acc_loss_my_method')
    plt.figure( figsize=(10, 8), dpi=600)
    plt.rcParams['font.sans-serif'] = ['times new roman']
    plt.rcParams['axes.unicode_minus'] = False  # 用来正常显示负号
    # 绘制曲线
    plt.plot(source_train_acc_list, label='source_train_acc', linestyle='-', color='blue')
    plt.plot(test_acc_list, label='target_train_acc', linestyle='-', color='green')
    # 添加标签和标题
    plt.xlabel('epoch')
    plt.ylabel('acc')
    plt.title('train_val_acc')
    plt.legend(loc='lower right')  # 左下显示
    fig_path = os.path.join(save_dir, 'all_acc.pdf')
    fig_path1 = os.path.join(save_dir, 'all_acc.png')
    plt.savefig(fig_path)
    plt.savefig(fig_path1)
    plt.close()

    plt.figure( figsize=(10, 8), dpi=600)
    plt.plot(source_total_losses_list, label='source_train_loss', linestyle='-', color='blue')
    plt.plot(class_losses_list, label='val_loss', linestyle='-', color='green')
    plt.plot(weighted_domain_losses_list, label='weighted_domain_loss', linestyle='-', color='orange')
    plt.plot(losses_list, label='val_loss', linestyle='--', color='red')
    plt.xlabel('epoch')
    plt.ylabel('loss')
    plt.title('all loss')
...
def test_plot(loader, feature_extractor_DANN, classification_extractor_DANN,  device):
    raw_feat_list, fc_feat_list, label_list = [], [], []
    feature_extractor_DANN.eval()
    classification_extractor_DANN.eval()
    with torch.no_grad():
        for data, label in loader:
            ...
            features = feature_extractor_DANN(data)
            outputs = classification_extractor_DANN(features)
...
def plot_tsne_my(args, save_dir, feature_extractor, classification_extractor, feature_extractor_name, classification_extractor_name):
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    print(f"Using device: {device}")
    print('plot_tsne_my_method')
    init_seed(args.seed)
    feature_extractor.to(device)
    classification_extractor.to(device)
    # Load pretrained weights
    feature_extractor.load_state_dict(torch.load(os.path.join(save_dir, feature_extractor_name)))
    classification_extractor.load_state_dict(torch.load(os.path.join(save_dir, classification_extractor_name)))
    ...
    # Extract features
    val_raw, val_fc, val_label = test_plot(val_loader, feature_extractor, classification_extractor,  device)
    src_raw, src_fc, src_label = test_plot(source_loader, feature_extractor, classification_extractor,  device)
