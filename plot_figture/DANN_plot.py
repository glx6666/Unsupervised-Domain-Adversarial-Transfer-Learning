...
def plot_DANN(source_total_acc_list, source_total_losses_list, val_acc_list, val_losses_list, cm, save_dir, fault_classes):
    print('plot_acc_loss_DANN')
    plt.figure( figsize=(10, 4), dpi=600)
    plt.rcParams['font.sans-serif'] = ['times new roman']
    plt.rcParams['axes.unicode_minus'] = False  # 用来正常显示负号
    #plt.subplot(1, 2, 1)
    # 绘制曲线
    plt.plot(source_total_acc_list, label='source_train_acc', linestyle='-', color='blue')
    plt.plot(val_acc_list, label='target_train_acc', linestyle='-', color='green')
    # 添加标签和标题
    plt.xlabel('epoch')
    plt.ylabel('acc')
    ...
    plt.plot(val_losses_list, label='val_loss', linestyle='-', color='green')
    plt.xlabel('epoch')
    plt.ylabel('loss')
    plt.title('all loss')
...
def plot_tsne_DANN(args, save_dir, feature_extractor_DANN, classification_extractor_DANN, feature_extractor_name, classification_extractor_name):
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    print(f"Using device: {device}")
    print('plot_tsne_DANN')
    init_seed(args.seed)
    feature_extractor_DANN.to(device)
    classification_extractor_DANN.to(device)
    # Load pretrained weights
    feature_extractor_DANN.load_state_dict(torch.load(os.path.join(save_dir, feature_extractor_name)))
    classification_extractor_DANN.load_state_dict(torch.load(os.path.join(save_dir, classification_extractor_name)))
    ...
    # Extract features
    val_raw, val_fc, val_label = test_plot(val_loader, feature_extractor_DANN, classification_extractor_DANN,  device)
    src_raw, src_fc, src_label = test_plot(source_loader, feature_extractor_DANN, classification_extractor_DANN,  device)
