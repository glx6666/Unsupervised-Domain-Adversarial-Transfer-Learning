...
def plot_DAN(source_acc_list, source_losses_list, val_acc_list, val_losses_list, cm, save_dir, fault_classes):
    print('plot_acc_DAN')
...
def test_plot(loader, feature_extractor_DAN, classification_extractor_DAN,  device):
    raw_feat_list, fc_feat_list, label_list = [], [], []
    feature_extractor_DAN.eval()
    classification_extractor_DAN.eval()
    with torch.no_grad():
        for data, label in loader:
            ...
            features = feature_extractor_DAN(data)
            outputs = classification_extractor_DAN(features)
...
def plot_tsne_DAN(args, save_dir, feature_extractor_DAN, classification_extractor_DAN, feature_extractor_name, classification_extractor_name):
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    print(f"Using device: {device}")
    init_seed(args.seed)
    feature_extractor_DAN.to(device)
    classification_extractor_DAN.to(device)
    # Load pretrained weights
    feature_extractor_DAN.load_state_dict(torch.load(os.path.join(save_dir, feature_extractor_name)))
    classification_extractor_DAN.load_state_dict(torch.load(os.path.join(save_dir, classification_extractor_name)))
    ...
    # Extract features
    val_raw, val_fc, val_label = test_plot(val_loader, feature_extractor_DAN, classification_extractor_DAN,  device)
    src_raw, src_fc, src_label = test_plot(source_loader, feature_extractor_DAN, classification_extractor_DAN,  device)
